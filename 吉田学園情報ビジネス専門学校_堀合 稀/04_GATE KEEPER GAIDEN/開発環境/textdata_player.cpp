...
bool CTextDataPlayer::m_bSetBullet[UNLOCKDATA_MAX];
	...
	{ //ファイル展開可能
		...
		while (strcmp("PLAYERDATA_END", pLoadText) != 0)
		{
			...
			{// 弾(波形)のアンロック
				...
				if (m_aPlayerData[PLAYERDATA_UNLOCK_BULLET_WAVE] != 0)
				{
					m_bSetBullet[UNLOCKDATA_WAVE] = true;
				}
			}
			...
			{// 弾(ミサイル)のアンロック
				...
				if (m_aPlayerData[PLAYERDATA_UNLOCK_BULLET_MISSILE] != 0)
				{
					m_bSetBullet[UNLOCKDATA_MISSILE] = true;
				}
			}
			...
			{// 弾(逆波形)のアンロック
				...
				if (m_aPlayerData[PLAYERDATA_UNLOCK_BULLET_REVERSE] != 0)
				{
					m_bSetBullet[UNLOCKDATA_REVERSE] = true;
				}
			}
		}
