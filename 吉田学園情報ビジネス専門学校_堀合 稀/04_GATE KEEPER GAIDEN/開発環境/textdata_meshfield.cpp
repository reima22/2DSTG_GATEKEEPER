...
{
	// ローカル変数宣言
	CTextDataMeshfield *pTextDataMeshfield;
	pTextDataMeshfield = new CTextDataMeshfield;	// インスタンス生成

								// テキストデータ
	if (pTextDataMeshfield != NULL)
	{
		// テキストデータの読み込み
		pTextDataMeshfield->LoadData();
	}

	return pTextDataMeshfield;
}
...
// テキストデータの破棄
void CTextDataMeshfield::UnloadData(void)
{
